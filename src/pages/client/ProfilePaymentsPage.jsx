    ...
    const fetchCards = async () => {
      try {
        ...
        const adapted = data.map((c) => ({
          id: c.idCard,
          terminacion: c.cardNumber.slice(-4),
    ...
    <main className="flex-1">
      <div className="max-w-4xl mx-auto px-8 pt-10 pb-10">
        ...
        <section className="bg-white border border-gray-200 rounded-2xl shadow-sm px-6 py-5">
          ...
          <p className="mt-1 mb-4 text-[0.85rem] text-gray-500">Administra tus tarjetas y métodos de pago</p>
            ...
            {metodos.map((t, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`${getColorClass(index)} w-10 h-10 rounded-xl flex items-center justify-center text-white text-lg`}>▢</div>
                  <div className="text-sm">
                    <p className="m-0 font-semibold text-gray-900">•••• •••• •••• {t.terminacion}</p>
